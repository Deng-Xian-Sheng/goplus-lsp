...
func getGopBuildFlags() string {
	...
	buildFlags := fmt.Sprintf("-X \"github.com/Deng-Xian-Sheng/goplus-lsp/gop/env.defaultGopRoot=%s\"", defaultGopRoot)
	buildFlags += fmt.Sprintf(" -X \"github.com/Deng-Xian-Sheng/goplus-lsp/gop/env.buildDate=%s\"", getBuildDateTime())

	version := findGopVersion()
	buildFlags += fmt.Sprintf(" -X \"github.com/Deng-Xian-Sheng/goplus-lsp/gop/env.buildVersion=%s\"", version)
